...
int_t ztrs_B_init3d(int_t nsupers, doublecomplex* x, int nrhs, zLUstruct_t * LUstruct,
	gridinfo3d_t *grid3d)
{
...
int_t ztrs_B_init3d_newsolve(int_t nsupers, doublecomplex* x, int nrhs, zLUstruct_t * LUstruct,
                           ...
                           int* supernodeMask, gridinfo_t *grid, SuperLUStat_t * stat)
{
	...
	doublecomplex **Lnzval_bc_ptr=Llu->Lnzval_bc_ptr;  /* size ceil(NSUPERS/Pc) */
            ...
            if(supernodeMask[jb]>0){
                ...
                if(Iactive==1){
                    ...
                    if(rank_cnt>1){
                        ...
                        int needrecv=0;
                        C_BcTree_Create_nv(&LBtree_ptr[lk], grid->comm, ranks, rank_cnt, msgsize, 'z',&needrecv);
            ...
            if(supernodeMask[ib]>0){
                ...
                if(Iactive==1){
                    ...
                    if(rank_cnt>1){
                        ...
                        // C_RdTree_Create(&LRtree_ptr[lk], grid->comm, ranks, rank_cnt, msgsize, 'z');

                        int needrecvrd=0;
                        int needsendrd=0;
                        C_RdTree_Create_nv(&LRtree_ptr[lk], grid->comm, ranks, rank_cnt, msgsize, 'z', &needrecvrd, &needsendrd);
                ...
                if(Iactive==1){
                    ...
                    if(rank_cnt>1){
                        for (int_t ii=0;ii<rank_cnt;ii++)   // use global ranks rather than local ranks
                            ranks[ii] = PNUM( ranks[ii], pc, grid );
                        msgsize = SuperSize( jb );
                        // C_BcTree_Create(&UBtree_ptr[lk], grid->comm, ranks, rank_cnt, msgsize, 'z');

                        int needrecv=0;
                        C_BcTree_Create_nv(&UBtree_ptr[lk], grid->comm, ranks, rank_cnt, msgsize, 'z',&needrecv);
                ...
                if(Iactive==1){
                    ...
                    if(rank_cnt>1){
                        for (int_t ii=0;ii<rank_cnt;ii++)   // use global ranks rather than local ranks
                            ranks[ii] = PNUM( pr, ranks[ii], grid );
                        msgsize = SuperSize( ib );
                        // C_RdTree_Create(&URtree_ptr[lk], grid->comm, ranks, rank_cnt, msgsize, 'z');

                        int needrecvrd=0;
                        int needsendrd=0;
                        C_RdTree_Create_nv(&URtree_ptr[lk], grid->comm, ranks, rank_cnt, msgsize, 'z', &needrecvrd,&needsendrd);
	...
	for (int_t ilvl = 1; ilvl < maxLvl ; ++ilvl)
	{
        if(!myZeroTrIdxs[ilvl-1]){ // this ensures the number of grids in communication is reduced by half every level down
            ...
            for (int_t alvl = ilvl; alvl < maxLvl; alvl++)
            {
                ...
                // fflush(stdout);
                zreduceSolvedX_newsolve(tr, sender, receiver, x, nrhs,  trf3Dpartition, LUstruct, grid3d, recvbuf, xtrsTimer);
                tr=(tr+1)/2-1;
	...
	for (int_t ilvl = maxLvl-1; ilvl >0 ; --ilvl)
	{
        if(!myZeroTrIdxs[ilvl-1]){ // this ensures the number of grids in communication is doubled every level down
            ...
            for (int_t alvl = ilvl; alvl < maxLvl; alvl++)
            {
                ...
                zp2pSolvedX3d(tr, sender, receiver, x, nrhs,  trf3Dpartition, LUstruct, grid3d, xtrsTimer);
                tr=(tr+1)/2-1;
...
int_t zreduceSolvedX_newsolve(int_t treeId, int_t sender, int_t receiver, doublecomplex* x, int nrhs,
                      ztrf3Dpartition_t*  trf3Dpartition, zLUstruct_t* LUstruct, gridinfo3d_t* grid3d, doublecomplex* recvbuf, xtrsTimer_t *xtrsTimer)
{
	...
	for (int_t k0 = 0; k0 < nnodes; ++k0)
	{
		...
		if (myrow == krow && mycol == kcol)
		{
			...
			{
				/* code */
				MPI_Send( &x[ii], knsupc * nrhs, SuperLU_MPI_DOUBLE_COMPLEX, receiver, k,  grid3d->zscp.comm);
                for(int_t i=0; i<knsupc * nrhs; i++){
                    x[ii+i]=zero;
                }
			...
			{
				MPI_Status status;
				MPI_Recv( recvbuf, knsupc * nrhs, SuperLU_MPI_DOUBLE_COMPLEX, sender, k, grid3d->zscp.comm, &status );
                for(int_t i=0; i<knsupc * nrhs; i++){
					z_add(&x[ii+i], &x[ii+i], &recvbuf[i]);
                }
...
int_t ztrs_X_gather3d(doublecomplex* x, int nrhs, ztrf3Dpartition_t*  trf3Dpartition,
...
int_t zp2pSolvedX3d(int_t treeId, int_t sender, int_t receiver, doublecomplex* x, int nrhs,
                      ztrf3Dpartition_t*  trf3Dpartition, zLUstruct_t* LUstruct, gridinfo3d_t* grid3d, xtrsTimer_t *xtrsTimer)
{
	...
	for (int_t k0 = 0; k0 < nnodes; ++k0)
	{
		...
		if (myrow == krow && mycol == kcol)
		{
			...
			{
				MPI_Status status;
				MPI_Recv( &x[ii], knsupc * nrhs, SuperLU_MPI_DOUBLE_COMPLEX, sender, k, grid3d->zscp.comm, &status );
                xtrsTimer->trsDataRecvZ += knsupc * nrhs;
            }
		}
	}
...
int_t zfsolveReduceLsum3d(int_t treeId, int_t sender, int_t receiver, doublecomplex* lsum, doublecomplex* recvbuf, int nrhs,
                         ztrf3Dpartition_t*  trf3Dpartition, zLUstruct_t* LUstruct, gridinfo3d_t* grid3d ,
	...
	for (int_t k0 = 0; k0 < nnodes; ++k0)
	{
		...
		if (myrow == krow )
		{
			...
			if (myGrid == sender)
			{
				...
				doublecomplex* lsum_k = &lsum[ii];
				superlu_scope_t *scp = &grid->rscp;
				MPI_Reduce( lsum_k, recvbuf, knsupc * nrhs,
				            SuperLU_MPI_DOUBLE_COMPLEX, MPI_SUM, kcol, scp->comm);
				xtrsTimer->trsDataSendXY += knsupc * nrhs;
				xtrsTimer->trsDataRecvXY += knsupc * nrhs;
				if (mycol == kcol)
				{
					MPI_Send( recvbuf, knsupc * nrhs, SuperLU_MPI_DOUBLE_COMPLEX, receiver, k,  grid3d->zscp.comm);
					xtrsTimer->trsDataSendZ += knsupc * nrhs;
				}
			}
			else
			{
				if (mycol == kcol)
				{
					...
					xtrsTimer->trsDataRecvZ += knsupc * nrhs;
					int_t ii = LSUM_BLK (lk);
					doublecomplex* dest = &lsum[ii];
					doublecomplex* tempv = recvbuf;
					for (int_t j = 0; j < nrhs; ++j)
					{
						for (int_t i = 0; i < knsupc; ++i)
					        z_add(&dest[i + j * knsupc], &dest[i + j * knsupc], &tempv[i + j * knsupc]);
                    }
				}
...
int_t zbsolve_Xt_bcast(int_t ilvl, zxT_struct *xT_s, int nrhs, ztrf3Dpartition_t*  trf3Dpartition,
			...
			for (int_t k0 = 0; k0 < nnodes ; ++k0)
			{
				...
				if (myGrid == sender)
				{
					...
					if (mycol == kcol &&   myrow == krow)
					{
						...
						int_t lk = LBj (k, grid);
						int_t ii = XT_BLK (lk);
						doublecomplex* xk = &xT[ii];
						MPI_Send( xk, knsupc * nrhs, SuperLU_MPI_DOUBLE_COMPLEX, receiver, k,
						           grid3d->zscp.comm);
						xtrsTimer->trsDataSendZ += knsupc * nrhs;
				...
				else
				{
					if (mycol == kcol)
					{
						...
						if (myrow == krow )
						{
							...
							doublecomplex* xk = &xT[ii];
							MPI_Status status;
							MPI_Recv( xk, knsupc * nrhs, SuperLU_MPI_DOUBLE_COMPLEX, sender,k,
							           grid3d->zscp.comm, &status);
							xtrsTimer->trsDataRecvZ += knsupc * nrhs;
						}
						zbCastXk2Pck( k,  xT_s,  nrhs, LUstruct, grid, xtrsTimer);
					}
...
int_t zlsumForestFsolve(int_t k,
                       doublecomplex *lsum, doublecomplex *x, doublecomplex* rtemp,  zxT_struct *xT_s, int    nrhs,
                       zLUstruct_t * LUstruct,
...
int_t znonLeafForestForwardSolve3d( int_t treeId,  zLUstruct_t * LUstruct,
                                   zScalePermstruct_t * ScalePermstruct,
                                   ...
                                   zSOLVEstruct_t * SOLVEstruct, SuperLUStat_t * stat, xtrsTimer_t *xtrsTimer)
{
	...
	for (int_t k0 = 0; k0 < nnodes; ++k0)
	{
		...
		if (myrow == krow )
		{
			...
			zlsumReducePrK(k, x, lsum, recvbuf, nrhs, LUstruct, grid,xtrsTimer);
		...
		if (mycol == kcol )
		{
			...
			if (myrow == krow )
			{
				/* Diagonal process. */
				double tx = SuperLU_timer_();
				zlocalSolveXkYk(  LOWER_TRI,  k,  &x[ii],  nrhs, LUstruct,   grid, stat);
				int_t lkj = LBj (k, grid);
				int_t jj = XT_BLK (lkj);
				int_t knsupc = SuperSize(k);
				memcpy(&xT[jj], &x[ii], knsupc * nrhs * sizeof(doublecomplex) );
			 ...
			 */
			tx = SuperLU_timer_();
			zlsumForestFsolve(k, lsum, x, rtemp, xT_s, nrhs,
...
int_t zleafForestForwardSolve3d(superlu_dist_options_t *options, int_t treeId, int_t n,  zLUstruct_t * LUstruct,
                               zScalePermstruct_t * ScalePermstruct,
                               ztrf3Dpartition_t*  trf3Dpartition, gridinfo3d_t *grid3d,
	...
	for (int_t k0 = 0; k0 < nnodes && nleaf; ++k0)
	{
		...
		if (myrow == krow && mycol == kcol)
		{
			...
			if (frecv[lk] == 0 && fmod[lk] == 0)
			{
				...
				zlocalSolveXkYk(  LOWER_TRI,  k,  &x[ii],  nrhs, LUstruct,   grid, stat);
				ziBcastXk2Pck( k,  &x[ii - XK_H],  nrhs, fsendx_plist, send_req, LUstruct, grid,xtrsTimer);
				nleaf--;
				/*
				...
				zlsum_fmod_leaf (treeId, trf3Dpartition, lsum, x, &x[ii], rtemp, nrhs, knsupc, k,
	...
	while (nfrecvx || nfrecvmod)
	{
		...
		case Xk:
		{
			...
			if (lsub)
			{
				 ...
				 * Perform local block modifications: lsum[i] -= L_i,k * X[k]
				 */
				zlsum_fmod_leaf (treeId, trf3Dpartition, lsum, x, &recvbuf[XK_H], rtemp, nrhs, knsupc, k,
				                 fmod, nb, lptr, luptr, xsup, grid, Llu,
				                 send_req, stat, xtrsTimer);
			}                   /* if lsub */
		...
		case LSUM:             /* Receiver must be a diagonal process */
		{
			...
			int_t ii = X_BLK (lk);
			int_t knsupc = SuperSize (k);
			doublecomplex* tempv = &recvbuf[LSUM_H];
			for (int_t j = 0; j < nrhs; ++j)
			{
				for (int_t i = 0; i < knsupc; ++i)
					z_add(&x[i + ii + j * knsupc], &x[i + ii + j * knsupc], &tempv[i + j * knsupc]);
			}
		...
		if ( (--fmod[lk]) == 0  )   /* Local accumulation done. */
		{
			if (trf3Dpartition->supernode2treeMap[ik] == treeId)
			{
				...
				else     /* Diagonal process: X[i] += lsum[i]. */
				{
					...
					RHS_ITERATE(j)
					for (i = 0; i < iknsupc; ++i)
					    z_add(&x[i + ii + j * iknsupc], &x[i + ii + j * iknsupc], &lsum[i + il + j * iknsupc]);
					...
					if ( frecv[lk] == 0 )   /* Becomes a leaf node. */
					{
						...
						zlsum_fmod_leaf(treeId, trf3Dpartition,
...
int_t zleafForestForwardSolve3d_newsolve(superlu_dist_options_t *options, int_t n,  zLUstruct_t * LUstruct,
                               zScalePermstruct_t * ScalePermstruct,
                               ztrf3Dpartition_t*  trf3Dpartition, gridinfo3d_t *grid3d,
                               doublecomplex * x, doublecomplex * lsum, doublecomplex * recvbuf, doublecomplex* rtemp,
                               MPI_Request * send_req,
	...
	for (int_t k = 0; k < nsupers && nleaf; ++k)
	{
		...
		if (myrow == krow && mycol == kcol)
		{
			...
			if (frecv[lk] == 0 && fmod[lk] == 0)
			{
				...
				zlsum_fmod_leaf_newsolve (trf3Dpartition, lsum, x, &x[ii], rtemp, nrhs, knsupc, k,
				                 fmod, nb, lptr, luptr, xsup, grid, Llu,
				                 send_req, stat, xtrsTimer);
				xtrsTimer->tfs_compute += SuperLU_timer_() - tx;
			}
	...
	while (nfrecvx || nfrecvmod)
	{
		...
		case Xk:
		{
			...
			if (lsub)
			{
				 ...
				 * Perform local block modifications: lsum[i] -= L_i,k * X[k]
				 */
				zlsum_fmod_leaf_newsolve (trf3Dpartition, lsum, x, &recvbuf[XK_H], rtemp, nrhs, knsupc, k,
				                 fmod, nb, lptr, luptr, xsup, grid, Llu,
				                 send_req, stat, xtrsTimer);
			}                   /* if lsub */
...
void zForwardSolve3d_newsolve_reusepdgstrs(superlu_dist_options_t *options, int_t n,  zLUstruct_t * LUstruct,
                               zScalePermstruct_t * ScalePermstruct,
		...
		if ( (--fmod[lk]) == 0  )   /* Local accumulation done. */
		{
            ...
            else     /* Diagonal process: X[i] += lsum[i]. */
            {
                ...
                if ( frecv[lk] == 0 )   /* Becomes a leaf node. */
                {
                    ...
                    zlsum_fmod_leaf_newsolve(trf3Dpartition,
                                    lsum, x, &x[ii], rtemp, nrhs, iknsupc, ik,
                                    fmod, nlb1, lptr1, luptr1, xsup,
                                    grid, Llu, send_req, stat,xtrsTimer);
                } /* if frecv[lk] == 0 */
...
int_t zlasum_bmod_Tree(int_t  pTree, int_t cTree, doublecomplex *lsum, doublecomplex *x,
                       zxT_struct *xT_s,
                       ...
                       gridinfo3d_t* grid3d, SuperLUStat_t * stat)
{
    ...
    for (int_t k0 = 0; k0 < nnodes; ++k0)
    {
        ...
        {
            /* code */
            zlsumForestBsolve(k, cTree, lsum, x, xT_s, nrhs, lbmod_buf,
                             LUstruct, trf3Dpartition, grid3d, stat);
        }
    }
    return 0;
...
int_t zpackXbmod( int_t knsupc, int_t ncols, int_t nrhs, int_t* indCols, doublecomplex* xk, doublecomplex* tempx)
{
    ...
    for (int_t j = 0; j < nrhs; ++j)
    {
        doublecomplex* dest = &tempx[j * ncols];
        doublecomplex* y = &xk[j * knsupc];
...
int_t zlsumBmod(int_t gik, int_t gjk, int nrhs, zlsumBmod_buff_t* lbmod_buf,
               int_t* usub,  doublecomplex* uval,
               doublecomplex* xk, doublecomplex* lsum, int_t* xsup, SuperLUStat_t * stat)
{

    int_t* indCols = lbmod_buf->indCols;
    doublecomplex* tempu = lbmod_buf->tU;
    doublecomplex* tempx = lbmod_buf->tX;
    ...
    int ncols = zpackUblock(ldu, indCols, knsupc, iklrow, usub,
                             tempu, uval );

    doublecomplex alpha = {-1.0, 0.0};
    doublecomplex beta = {1.0, 0.0};
    doublecomplex* X;

    if (ncols < knsupc)
    {
        /* code */
        zpackXbmod(knsupc, ncols, nrhs, indCols, xk, tempx);
        X = tempx;
    }
    else
    {
    ...
    doublecomplex* V = &lsum[iknsupc - ldu];


#if defined (USE_VENDOR_BLAS)
	zgemm_("N", "N", &ldu, &nrhs, &ncols, &alpha,
	tempu, &ldu,
...
int_t zlsumForestBsolve(int_t k, int_t treeId,
                       ...
                       gridinfo3d_t* grid3d, SuperLUStat_t * stat)
{
    ...
    for (int_t ub = 0; ub < nub; ++ub)
    {
        ...
        if (trf3Dpartition->supernode2treeMap[gik] == treeId)
        {
            ...
            for (int_t j = 0; j < nrhs; ++j)
            {
                doublecomplex* dest = &lsum[il + j * iknsupc];
                doublecomplex* y = &xk[j * knsupc];
...
int_t  zbCastXk2Pck  (int_t k, zxT_struct *xT_s, int nrhs,
                     zLUstruct_t * LUstruct, gridinfo_t * grid, xtrsTimer_t *xtrsTimer)
{
    ...
    int_t knsupc = SuperSize (k);
    int_t krow = PROW (k, grid);
    MPI_Bcast( xk, knsupc * nrhs, SuperLU_MPI_DOUBLE_COMPLEX, krow,
               scp->comm);
...
int_t  zlsumReducePrK (int_t k, doublecomplex*x, doublecomplex* lsum, doublecomplex* recvbuf, int nrhs,
                      zLUstruct_t * LUstruct, gridinfo_t * grid, xtrsTimer_t *xtrsTimer)
{
    /*
     * Send Xk to process column Pc[k].
...
int_t znonLeafForestBackSolve3d( int_t treeId,  zLUstruct_t * LUstruct,
                                zScalePermstruct_t * ScalePermstruct,
                                ztrf3Dpartition_t*  trf3Dpartition, gridinfo3d_t *grid3d,
                                doublecomplex * x, doublecomplex * lsum,
                                zxT_struct *xT_s,
    ...
    for (int_t k0 = nnodes - 1; k0 >= 0; --k0)
    {
        ...
        if (mycol == kcol )
        {
            ...
            if (myrow == krow )
            {
                double tx = SuperLU_timer_();
                /* Diagonal process. */
                zlocalSolveXkYk(  UPPER_TRI,  k,  &x[ii],  nrhs, LUstruct,   grid, stat);
                int_t lkj = LBj (k, grid);
                int_t jj = XT_BLK (lkj);
                int_t knsupc = SuperSize(k);
                memcpy(&xT[jj], &x[ii], knsupc * nrhs * sizeof(doublecomplex) );
             ...
             */
            tx = SuperLU_timer_();
            zlsumForestBsolve(k, treeId, lsum, x, xT_s, nrhs, lbmod_buf,
                             LUstruct, trf3Dpartition, grid3d, stat);
            xtrsTimer->tbs_compute += SuperLU_timer_() - tx;
        }
    }                           /* for k ... */
...
int_t zleafForestBackSolve3d(superlu_dist_options_t *options, int_t treeId, int_t n,  zLUstruct_t * LUstruct,
                            zScalePermstruct_t * ScalePermstruct,
                            ztrf3Dpartition_t*  trf3Dpartition, gridinfo3d_t *grid3d,
                            ...
                            zSOLVEstruct_t * SOLVEstruct, SuperLUStat_t * stat, xtrsTimer_t *xtrsTimer)
{
        ...
        if (myrow == krow)
        {
            ...
            if (bmod[lk] == 0)
            {
                ...
                else
                {
                    ...
                    for (int_t j = 0; j < nrhs; ++j)
                        for (int_t i = 0; i < knsupc; ++i)
					        z_add(&dest[i + j * knsupc], &dest[i + j * knsupc], &lsum[i + il + j * knsupc]);
                    ...
                    if (brecv[lk] == 0 )
                    {
                         ...
                         * Send Xk to process column Pc[k].
                         */
                        ziBcastXk2Pck( k,  &x[ii - XK_H],  nrhs, bsendx_plist, send_req, LUstruct, grid,xtrsTimer);
                        /*
                         * Perform local block modifications: lsum[i] -= U_i,k * X[k]
                         */
                        if (Urbs[lkj])
                            zlsum_bmod_GG (lsum, x, &x[ii], nrhs, lbmod_buf,  k, bmod, Urbs,
    ...
    while (nbrecvx || nbrecvmod)
    {
        ...
        case Xk:
        {
             ...
             *         lsum[i] -= U_i,k * X[k]
             */
            zlsum_bmod_GG (lsum, x, &recvbuf[XK_H], nrhs, lbmod_buf, k, bmod, Urbs,
                           Ucb_indptr, Ucb_valptr, xsup, grid, Llu,
                           send_req, stat,xtrsTimer);
            break;
        }
...
int_t zleafForestBackSolve3d_newsolve(superlu_dist_options_t *options, int_t n,  zLUstruct_t * LUstruct,
                            ztrf3Dpartition_t*  trf3Dpartition, gridinfo3d_t *grid3d,
                            doublecomplex * x, doublecomplex * lsum, doublecomplex * recvbuf,
                            MPI_Request * send_req,
                            int nrhs, zlsumBmod_buff_t* lbmod_buf,
                            zSOLVEstruct_t * SOLVEstruct, SuperLUStat_t * stat, xtrsTimer_t *xtrsTimer)
{
            ...
            if (bmod[lk] == 0)
            {
                ...
                if (brecv[lk] == 0 )
                {
                    ...
                    /*
                        ...
                        */
                    if (Urbs[lkj])
                        zlsum_bmod_GG_newsolve (trf3Dpartition, lsum, x, &x[ii], nrhs, lbmod_buf,  k, bmod, Urbs,
                                        Ucb_indptr, Ucb_valptr, xsup, grid, Llu,
                                        send_req, stat,xtrsTimer);
                    xtrsTimer->tbs_compute += SuperLU_timer_() - tx;
                }                   /* if root ... */
    ...
    while (nbrecvx || nbrecvmod)
    {
        ...
        case Xk:
        {
             ...
             *         lsum[i] -= U_i,k * X[k]
             */
            zlsum_bmod_GG_newsolve (trf3Dpartition, lsum, x, &recvbuf[XK_H], nrhs, lbmod_buf, k, bmod, Urbs,
                           Ucb_indptr, Ucb_valptr, xsup, grid, Llu,
                           send_req, stat,xtrsTimer);
            break;
        }
...
void zBackSolve3d_newsolve_reusepdgstrs(superlu_dist_options_t *options, int_t n,  zLUstruct_t * LUstruct,
                               int*  supernodeMask, gridinfo3d_t *grid3d,
                               doublecomplex * x, doublecomplex * lsum,
                               int nrhs,
                               zSOLVEstruct_t * SOLVEstruct, SuperLUStat_t * stat, xtrsTimer_t *xtrsTimer)
    ...
    for (ub = 0; ub < nub; ++ub)
    {
        ...
        if ( (--bmod[ik]) == 0 )   /* Local accumulation done. */
        {
            ...
            else     /* Diagonal process: X[i] += lsum[i]. */
            {
                ...
                for (int_t j = 0; j < nrhs; ++j)
                    for (i = 0; i < iknsupc; ++i)
					    z_add(&dest[i + j * iknsupc], &dest[i + j * iknsupc], &lsum[i + il + j * iknsupc]);
                if ( !brecv[ik] )   /* Becomes a leaf node. */
                {
                     ...
                     */
                    if ( Urbs[lk1] )
                        zlsum_bmod_GG(lsum, x, &x[ii], nrhs, lbmod_buf, gik, bmod, Urbs,
                                      Ucb_indptr, Ucb_valptr, xsup, grid, Llu,
                                      send_req, stat,xtrsTimer);
                } /* if brecv[ik] == 0 */
            }
        ...
        if ( (--bmod[ik]) == 0 )   /* Local accumulation done. */
        {
            ...
            else     /* Diagonal process: X[i] += lsum[i]. */
            {
                ...
                if ( !brecv[ik] )   /* Becomes a leaf node. */
                {
                     ...
                     */
                    if ( Urbs[lk1] )
                        zlsum_bmod_GG_newsolve(trf3Dpartition, lsum, x, &x[ii], nrhs, lbmod_buf, gik, bmod, Urbs,
                                      Ucb_indptr, Ucb_valptr, xsup, grid, Llu,
                                      send_req, stat,xtrsTimer);
                } /* if brecv[ik] == 0 */
            }
...
int_t zlocalSolveXkYk( trtype_t trtype, int_t k, doublecomplex* x, int nrhs,
...
int_t ziBcastXk2Pck(int_t k, doublecomplex* x, int nrhs,
...
#if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid->iam, "Enter pzReDistribute3d_B_to_X()");
#endif
...
void
pzgstrs3d (superlu_dist_options_t *options, int_t n, zLUstruct_t * LUstruct,
           zScalePermstruct_t * ScalePermstruct,
           ...
           zSOLVEstruct_t * SOLVEstruct, SuperLUStat_t * stat, int *info)
{
    ...
    *info = 0;
    ...
    maxrecvsz = knsupc * nrhs + SUPERLU_MAX (XK_H, LSUM_H);
    if (!
            (lsum = doublecomplexCalloc_dist (((size_t) ldalsum) * nrhs + nlb * LSUM_H)))
    ...
    doublecomplex* xT;
    if (!(xT = doublecomplexMalloc_dist (ldaspaT * nrhs + nub * XK_H)))
    ...
    ztrs_B_init3d(nsupers, x, nrhs, LUstruct, grid3d);
    ...
    //                 printf("before pzgsTrForwardSolve3d: lk %5d, k %5d, x[ii] %15.6f iam %5d \n",lk,k,x[ii+knsupc-1],grid3d->iam);
    ...
    pzgsTrForwardSolve3d(options, n,  LUstruct, ScalePermstruct, trf3Dpartition, grid3d, x,  lsum, &xT_s,
                          recvbuf, send_req,  nrhs, SOLVEstruct,  stat, &xtrsTimer);
    // pzgsTrForwardSolve3d_2d( n,  LUstruct, ScalePermstruct, trf3Dpartition, grid3d, x,  lsum, &xT_s,
     ...
     *---------------------------------------------------*/
    tx = SuperLU_timer_();
    pzgsTrBackSolve3d(options, n,  LUstruct, ScalePermstruct, trf3Dpartition, grid3d, x,  lsum, &xT_s,
                       recvbuf, send_req,  nrhs, SOLVEstruct,  stat, &xtrsTimer);
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC (iam, "Exit pzgstrs3d()");
#endif
...
#pragma omp parallel default(shared) private(ii)
    ...
    //                 printf("before pzgsTrForwardSolve3d_newsolve: lk %5d, k %5d, x[ii] %15.6f iam %5d \n",lk,k,x[ii+knsupc-1],grid3d->iam);
    ...
    pzgsTrForwardSolve3d_newsolve(options, n,  LUstruct, ScalePermstruct, trf3Dpartition, grid3d, x,  lsum,
                          recvbuf, send_req,  nrhs, SOLVEstruct,  stat, &xtrsTimer);
    xtrsTimer.t_forwardSolve = SuperLU_timer_() - tx;
    ...
    //                 printf("before ztrs_x_reduction_newsolve: lk %5d, k %5d, x[ii] %15.6f iam %5d \n",lk,k,x[ii+knsupc-1],grid3d->iam);
    ...
    tx = SuperLU_timer_();
    ztrs_x_reduction_newsolve(nsupers, x, nrhs, LUstruct, grid3d, trf3Dpartition, recvbuf, &xtrsTimer);
    ztrs_x_broadcast_newsolve(nsupers, x, nrhs, LUstruct, grid3d, trf3Dpartition, recvbuf, &xtrsTimer);
     ...
     *---------------------------------------------------*/
    tx = SuperLU_timer_();
    pzgsTrBackSolve3d_newsolve(options, n,  LUstruct, trf3Dpartition, grid3d, x,  lsum,
                       recvbuf, send_req,  nrhs, SOLVEstruct,  stat, &xtrsTimer);

    // printf("pzgsTrBackSolve3d_newsolve Llu->SolveMsgSent %10d size %10d\n",Llu->SolveMsgSent,SUPERLU_MAX (Llu->nfsendx, Llu->nbsendx) + nlb);
    // {
    // int_t maxLvl = log2i(grid3d->zscp.Np) + 1;
	// for (int_t ilvl = 0; ilvl < maxLvl ; ++ilvl)
	// {
...
int_t pzgsTrForwardSolve3d(superlu_dist_options_t *options, int_t n, zLUstruct_t * LUstruct,
                           zScalePermstruct_t * ScalePermstruct,
                           ztrf3Dpartition_t*  trf3Dpartition, gridinfo3d_t *grid3d,
        ...
        if (!myZeroTrIdxs[ilvl])
        {
            ...
            if (ilvl != maxLvl - 1)
            {
                ...
                for (int_t alvl = ilvl + 1; alvl <  maxLvl; ++alvl)
                {
                    ...
                    zfsolveReduceLsum3d(treeId, sender, receiver, lsum3d, recvbuf, nrhs,
...
int_t pzgsTrForwardSolve3d_newsolve(superlu_dist_options_t *options, int_t n, zLUstruct_t * LUstruct,
                           zScalePermstruct_t * ScalePermstruct,
                           ztrf3Dpartition_t*  trf3Dpartition, gridinfo3d_t *grid3d,
                           doublecomplex *x3d, doublecomplex *lsum3d,
                           doublecomplex * recvbuf,
...
if (get_new3dsolvetreecomm()){
    zForwardSolve3d_newsolve_reusepdgstrs(options, n, LUstruct,
                                ScalePermstruct, trf3Dpartition->supernodeMask, grid3d,
                                x3d, lsum3d, nrhs, SOLVEstruct, stat, xtrsTimer);
}else{
    zleafForestForwardSolve3d_newsolve(options, n, LUstruct,
                                ScalePermstruct, trf3Dpartition, grid3d,
                                x3d,  lsum3d, recvbuf, rtemp,
                                send_req,  nrhs, SOLVEstruct,  stat, xtrsTimer);
}
...
int_t pzgsTrBackSolve3d(superlu_dist_options_t *options, int_t n, zLUstruct_t * LUstruct,
                        zScalePermstruct_t * ScalePermstruct,
                        ztrf3Dpartition_t*  trf3Dpartition, gridinfo3d_t *grid3d,
                        doublecomplex *x3d, doublecomplex *lsum3d,
                        zxT_struct *xT_s,
                        ...
                        zSOLVEstruct_t * SOLVEstruct, SuperLUStat_t * stat, xtrsTimer_t *xtrsTimer)
{
    // printf("Using pzgsTrBackSolve3d_2d \n");
    ...
    zinitLsumBmod_buff(nsupc, nrhs, &lbmod_buf);
    ...
    for (int_t ilvl = maxLvl - 1; ilvl >= 0  ; --ilvl)
    {
        ...
        {
            double tx = SuperLU_timer_();
            zbsolve_Xt_bcast(ilvl, xT_s, nrhs, trf3Dpartition,
            ...
            while (trParent > -1 )
            {
                zlasum_bmod_Tree(trParent, tree, lsum3d, x3d,  xT_s, nrhs, &lbmod_buf,
                                 LUstruct, trf3Dpartition, grid3d, stat);
...
int_t pzgsTrBackSolve3d_newsolve(superlu_dist_options_t *options, int_t n, zLUstruct_t * LUstruct,
                        ztrf3Dpartition_t*  trf3Dpartition, gridinfo3d_t *grid3d,
                        doublecomplex *x3d, doublecomplex *lsum3d,
                        doublecomplex * recvbuf,
                        MPI_Request * send_req, int nrhs,
...
if (get_new3dsolvetreecomm()){
    zBackSolve3d_newsolve_reusepdgstrs(options, n, LUstruct,
                                trf3Dpartition->supernodeMask, grid3d,
                                x3d, lsum3d, nrhs, SOLVEstruct, stat, xtrsTimer);
}else{
    zleafForestBackSolve3d_newsolve(options, n, LUstruct, trf3Dpartition, grid3d, x3d,  lsum3d, recvbuf,
