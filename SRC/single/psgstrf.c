       ...
       sLUstruct_t * LUstruct, gridinfo_t * grid, SuperLUStat_t * stat, int *info)
{
    ...
    *info = 0;
    ...
    if (! (Lval_buf_2 = (float **)SUPERLU_MALLOC ((1 + num_look_aheads) * sizeof (float *))))
        ABORT ("Malloc fails for Lval_buf_2[].");
    if (! (Usub_buf_2 = (int_t **)SUPERLU_MALLOC ((1 + num_look_aheads) * sizeof (int_t *))))
        ABORT ("Malloc fails for Uval_buf_2[].");
    if (! (Uval_buf_2 = (float **)SUPERLU_MALLOC ((1 + num_look_aheads) * sizeof (float *))))
...
#ifdef HAVE_SYCL
        bigU = nullptr;
        bigU = sycl::malloc_host<float>(size_t(bigu_size), *(sycl_get_queue()));
        if (bigU == nullptr)
          ABORT("Malloc fails for dgemm buffer U ");
        bigV = nullptr;
        bigV = sycl::malloc_host<float>(size_t(bigv_size), *(sycl_get_queue()));
        if (bigV == nullptr)
...
/************************************************************************/
	...
	/*******************************************************************/
       ...
       if (superlu_acc_offload)
       {
         ...
         #include "sSchCompUdt-2Ddynamic.c"  // This code has better OpenMP support
       }
#else
