...
void psgsmv_init_fp64  // X values are DOUBLE
(
 ...
 )
{
    ...
    float *nzval, t;
    double *val_tosend = NULL, *val_torecv = NULL; // X values are DOUBLE
...
} /* end psgsmv_init_fp64 */


/*
 * Performs sparse matrix-vector multiplication.
 *     local dot-product is accumulated in double precision.
 *     Input and output are in single precision.
 * NOTE: gsmv_comm {..} the vector values to communicate are DOUBLE
 */
void
psgsmv_d2(
	  int  abs,                 /* Input. Do abs(A)*abs(x). */
...
)
{
    ...
    float *nzval;
    double *val_tosend, *val_torecv; // FIX: X values are DOUBLE
    MPI_Request *send_req, *recv_req;
    MPI_Status status;

    // Internal accumulation is in DOUBLE 
    double prod, sum, zero = 0.0;
    ...
    /* ------------------------------------------------------------
       COPY THE X VALUES INTO THE SEND BUFFER. (X is double)
       ------------------------------------------------------------*/
    ...
    /* ------------------------------------------------------------
       COMMUNICATE THE X VALUES. (X is double)
       ------------------------------------------------------------*/
    if ( !(send_req = (MPI_Request *)
	   SUPERLU_MALLOC(2*procs *sizeof(MPI_Request))))
        ABORT("Malloc fails for recv_req[].");
    ...
    if ( abs ) { /* Perform abs(A)*abs(x) */
		...
		prod = fabs((double) nzval[j]) * fabs((double) x[jcol]);  // in double 
		sum += prod;
	    }
	    ax[i] = sum;
        }
	    ...
	    for (j = extern_start[i]; j < rowptr[i+1]; ++j) {
	        jcol = colind[j];
	        prod = fabs((double) nzval[j]) * fabs((double) val_torecv[jcol]);
		sum += prod;
	    }
	    ax[i] += sum;
	}
	    ...
	    for (j = rowptr[i]; j < extern_start[i]; ++j) {
	        jcol = colind[j];
		prod = (double) nzval[j] * (double) x[jcol];
		sum += prod;
	    }
	    ax[i] = sum;
        }
	    ...
	    for (j = extern_start[i]; j < rowptr[i+1]; ++j) {
	        ...
	        prod = (double) nzval[j] * (double) val_torecv[jcol];
		sum += prod;
	    }
	    ax[i] += sum;
	}
