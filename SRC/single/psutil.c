...
void sLUstructFree(sLUstruct_t *LUstruct)
{
    ...
    sDestroy_trf3Dpartition(LUstruct->trf3Dpart);
...
int_t
psgstrs_init_device_lsum_x(superlu_dist_options_t *options, int_t n, int_t m_loc, int_t nrhs, gridinfo_t *grid,
	     sLUstruct_t *LUstruct, sSOLVEstruct_t *SOLVEstruct, int* supernodeMask)
{
#if ( defined(GPU_ACC) )
    float zero = 0.0;
    ...
    checkGPU(gpuMalloc( (void**)&(SOLVEstruct->d_lsum), sizelsum * sizeof(float)));
    checkGPU(gpuMalloc( (void**)&(SOLVEstruct->d_x), (ldalsum * nrhs + nlb * XK_H) * sizeof(float)));
    checkGPU(gpuMemset( SOLVEstruct->d_lsum, 0, sizelsum * sizeof(float)));
    checkGPU(gpuMemset( SOLVEstruct->d_x, 0, (ldalsum * nrhs + nlb * XK_H) * sizeof(float)));

    float* lsum = (float*)SUPERLU_MALLOC(sizelsum * sizeof(float));
    ...
    checkGPU(gpuMalloc( (void**)&(SOLVEstruct->d_lsum_save), sizelsum * sizeof(float)));
    checkGPU(gpuMemcpy(SOLVEstruct->d_lsum_save, lsum, sizelsum * sizeof(float), gpuMemcpyHostToDevice));
    SUPERLU_FREE(lsum);
    ...
    return 0;
} /* psgstrs_init_device_lsum_x */


int_t
psgstrs_delete_device_lsum_x(sSOLVEstruct_t *SOLVEstruct)
{
#if ( defined(GPU_ACC) )
...
void sDestroy_A3d_gathered_on_2d(sSOLVEstruct_t *SOLVEstruct, gridinfo3d_t *grid3d)
{
    ...
    SUPERLU_FREE( A2d );         // free 2D structure
    SUPERLU_FREE( A3d );         // free 3D structure
} /* sDestroy_A3d_gathered_on_2d_allgrid */
#endif
