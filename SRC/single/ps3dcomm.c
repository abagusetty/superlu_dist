...
int_t sAllocLlu(int_t nsupers, sLUstruct_t * LUstruct, gridinfo3d_t* grid3d)
{
	...
	float** Linv_bc_ptr = (float**)SUPERLU_MALLOC(nbc * sizeof(float*));
	float** Uinv_bc_ptr = (float**)SUPERLU_MALLOC(nbc * sizeof(float*));
...
/* Copies LU structure from layer 0 to all the layers */
{
    ...
    sscatter3dLPanels( nsupers, LUstruct, grid3d, supernodeMask);

    /*scatter all the U blocks and indexes*/
    sscatter3dUPanels( nsupers, LUstruct, grid3d, supernodeMask);
                       ...
                       sLUstruct_t * LUstruct, gridinfo3d_t* grid3d, int *supernodeMask)
{
	...
	float** Lnzval_bc_ptr = Llu->Lnzval_bc_ptr;
	float **Linv_bc_ptr = Llu->Linv_bc_ptr;
	float **Uinv_bc_ptr = Llu->Uinv_bc_ptr;
    int_t iam = grid->iam;
...
#if (DEBUGlevel >= 1)
    CHECK_MALLOC(grid3d->iam, "Enter sgatherAllFactoredLU");
#endif
...
#if (DEBUGlevel >= 1)
    CHECK_MALLOC(grid3d->iam, "Exit sgatherAllFactoredLU");
#endif
	...
	for (int_t ilvl = maxLvl-1; ilvl >0 ; --ilvl)
	{
        if(!myZeroTrIdxs[ilvl-1]){ // this ensures the number of grids in communication is doubled every level down
			...
			for (int_t alvl = ilvl; alvl < maxLvl; alvl++)
			{
				...
				if (sforest){
				sgatherFactoredLU(sender, receiver, gNodeCount[tr], gNodeLists[tr],
