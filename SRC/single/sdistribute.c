...
//#endif
	...
	ilsum = Llu->ilsum;
	ldaspa = Llu->ldalsum;
	if ( !(dense = floatCalloc_dist(((size_t)ldaspa) * sp_ienv_dist(3, options))) )
	...
	mem_use += 2.0*nrbu*iword + ldaspa * sp_ienv_dist(3, options) * sword;
...
#if ( PRNTlevel>=1 )
	mem_use += 6.0*k*iword + ldaspa * sp_ienv_dist(3, options) * sword;
#endif
