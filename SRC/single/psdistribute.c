...
float
psdistribute(superlu_dist_options_t *options, int_t n, SuperMatrix *A,
...
//#endif
	...
	ilsum = Llu->ilsum;
	ldaspa = Llu->ldalsum;
	if ( !(dense = floatCalloc_dist(ldaspa * sp_ienv_dist(3, options))) )
...
//#endif
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Enter psdistribute_allgrid()");
#endif
#if ( PROFlevel>=1 )
    t = SuperLU_timer_();
#endif
	...
	for (lb = 0; lb < nrbu; ++lb) {
		...
		if(supernodeMask[ib]>0){ // YL: added supernode mask here
			if ( !(index = intMalloc_dist(len1+1)) )
				ABORT("Malloc fails for Uindex[].");
			Ufstnz_br_ptr[lb] = index;
			if ( !(Unzval_br_ptr[lb] = floatMalloc_dist(len)) )
	...
	for (jb = 0; jb < nsupers; ++jb) { /* for each block column ... */
		...
		if ( nrbl) { /* Do not ensure the blocks are sorted! */
			if(supernodeMask[jb]>0){ // YL: added supernode mask here
		    /* Set up the initial pointers for each block in
		       index[] and nzval[]. */
		    /* Add room for descriptors */
		    len1 = len + BC_HEADER + nrbl * LB_DESCRIPTOR;
...
/* Count the nnzs per block column */
    ...
    /* Memory allocated but not freed:
       ilsum, fmod, fsendx_plist, bmod, bsendx_plist  */
    CHECK_MALLOC(iam, "Exit psdistribute_allgrid()");
#endif

    return (mem_use+memTRS);

} /* PSDISTRIBUTE_ALLGRID */

float
psdistribute_allgrid_index_only(superlu_dist_options_t *options, int_t n, SuperMatrix *A,
	     sScalePermstruct_t *ScalePermstruct,
	     Glu_freeable_t *Glu_freeable, sLUstruct_t *LUstruct,
	     gridinfo_t *grid, int* supernodeMask)
/*
...
//#endif
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Enter psdistribute_allgrid_index_only()");
#endif
#if ( PROFlevel>=1 )
    t = SuperLU_timer_();
#endif
	...
	for (jb = 0; jb < nsupers; ++jb) { /* for each block column ... */
		...
		for (j = fsupc; j < FstBlockC( jb+1 ); ++j) {
			...
			gb = BlockNum( irow );
			pr = PROW( gb, grid );
			if ( myrow == pr) { // YL: added supernode mask here
			    if(supernodeMask[gb]>0){
					lb = LBi( gb, grid ); /* Local block number */
					index = Ufstnz_br_ptr[lb];
					fsupc1 = FstBlockC( gb+1 );
    ...
    /* Memory allocated but not freed:
       ilsum, fmod, fsendx_plist, bmod, bsendx_plist  */
    CHECK_MALLOC(iam, "Exit psdistribute_allgrid_index_only()");
#endif
