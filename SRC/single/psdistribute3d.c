 ...
 */
{
    ...
    strf3Dpartition_t *trf3Dpart = LUstruct->trf3Dpart; /* Data structure containing 3D partition info */
...
//#endif
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Enter psdistribute3d_Yang()");
#endif
#if ( PROFlevel>=1 )
    t = SuperLU_timer_();
#endif
	...
	memTRS += k*sizeof(int_t*) + 2.0*k*sizeof(float*) + k*iword;  //acount for Lindval_loc_bc_ptr, Unnz, Linv_bc_ptr,Uinv_bc_ptr
...
/* Count the nnzs per block column */
    ...
    CHECK_MALLOC(iam, "Exit psdistribute3d_Yang()");
#endif
