     ...
     int ldb, int nrhs, gridinfo3d_t *grid3d, int *info)
{
    ...
    else if (A->nrow != A->ncol || A->nrow < 0 || A->Stype != SLU_NR_loc || A->Dtype != SLU_S || A->Mtype != SLU_GE)
        *info = -2;
...
void sscaleBoth(int_t m_loc, int_t fst_row, int_t *rowptr,
    int_t *colind, float *a, float *R, float *C) {
    int_t irow = fst_row;
    int_t icol;
    for (int_t j = 0; j < m_loc; ++j) {
        for (int_t i = rowptr[j]; i < rowptr[j + 1]; ++i) {
...
void sscalePrecomputed(SuperMatrix *A, sScalePermstruct_t *ScalePermstruct) {
    ...
    case NOEQUIL:
        break;
    case ROW:
        sscaleRows(m_loc, fst_row, rowptr, a, R);
        break;
    case COL:
        sscaleColumns(m_loc, rowptr, colind, a, C);
        break;
    case BOTH:
        sscaleBoth(m_loc, fst_row, rowptr, colind, a, R, C);
        break;
    default:
        break;
    }
...
void sscaleFromScratch(
    SuperMatrix *A, sScalePermstruct_t *ScalePermstruct,
                            ...
                            int *rowequ, int *colequ, int *iinfo)
{
    ...
    if (Fact == SamePattern_SameRowPerm) {
        sscalePrecomputed(A, ScalePermstruct);
    } else {
        sscaleFromScratch(A, ScalePermstruct, grid, rowequ, colequ, iinfo);
    }
 ...
 */
void sfindRowPerm_MC64(gridinfo_t* grid, int_t job,
                      int_t m, int_t n,
                      int_t nnz,
                      int_t* colptr,
                      int_t* rowind,
    ...
    if (grid->iam == root) {
        *iinfo = sldperm_dist(job, m, nnz, colptr, rowind, a_GA, perm_r, R1, C1);
    }
    ...
    if (*iinfo == 0) {
        ...
        if (job == 5 && Equil) {
            MPI_Bcast(R1, m, MPI_FLOAT, root, grid->comm);
            MPI_Bcast(C1, n, MPI_FLOAT, root, grid->comm);
        }
    }
}
 ...
 */
void sscale_distributed_matrix(int rowequ, int colequ, int_t m, int_t n,
 int_t m_loc, int_t *rowptr, int_t *colind, int_t fst_row, float *a,
  float *R, float *C, float *R1, float *C1)
{
    #if ( DEBUGlevel>=1 )
    LOG_FUNC_ENTER();
    #endif
 ...
 *
 * @note The functions sfindRowPerm_MC64, dscale_distributed_matrix and dpermute_global_A are called in this function.
 */
void sperform_LargeDiag_MC64(
    superlu_dist_options_t *options, fact_t Fact,
    sScalePermstruct_t *ScalePermstruct, sLUstruct_t *LUstruct,
    ...
    float *a_GA = (float *)(GAstore)->nzval;

    if (job == 5) {
        R1 = floatMalloc_dist(m);
        if (!R1)
            ABORT("SUPERLU_MALLOC fails for R1[]");
        C1 = floatMalloc_dist(n);
        if (!C1)
            ABORT("SUPERLU_MALLOC fails for C1[]");
    }

    // int iinfo;
    sfindRowPerm_MC64(grid, job, m, n,
    nnz,
    colptr,
    rowind,
 ...
 */
int sDistributePermutedMatrix(const superlu_dist_options_t *options,
                             const int_t n,
                             const SuperMatrix *A,
                             const sScalePermstruct_t *ScalePermstruct,
